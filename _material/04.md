...
Jakolaskun toiminta saattaa aiheuttaa yllätyksiä:
...
Jos operaattorin `/` molemmilla puolilla on kokonaisluku, tulos on kokonaisluku. Yllä laskun `5 / 4` tuloksena on `1` (eikä `1.25`), koska `5` ja `4` ovat kokonaislukuja. Kuitenkin jos ainakin toinen luvuista on desimaaliosan sisältävä liukuluku (kuten yllä `5.0`), tulos on liukuluku.
...
Komennon `UNION` avulla tulostaulu voidaan luoda yhdistämällä useamman `SELECT`-kyselyn tuottamat tulostaulut.

Oletetaan, että taulun `Teachers` sisältönä on:
...
Taulun `Students` sisältönä on puolestaan:
...
Nyt voimme hakea kaikki nimet molemmista tauluista näin:
...
Komennon `WITH` avulla kyselyn avuksi voidaan luoda nimetty apukysely. Tämä tarkoittaa, että kyselyn osana olevalle `SELECT`-kyselylle voidaan antaa nimi, jolla siihen voidaan viitata kyselyn aikana. Tällaisesta kyselystä käytetään usein nimeä _common table expression_ (_CTE_).
...
Tavoitteena on laskea jokaiselle asiakkaalle hänen ostostensa yhteissumma sekä lisäksi yhteissumma 20 prosentin alennuksella. Voimme toteuttaa kyselyn luvun 3 menetelmillä seuraavasti:

```sql
SELECT
  C.name,
  IFNULL(SUM(P.price), 0) AS total,
  IFNULL(SUM(P.price), 0) * 0.8 AS discount
FROM Customers AS C
...
Tämä on toimiva kysely, mutta kysely on melko monimutkainen ja siinä toistuu kahdesti `IFNULL(SUM(P.price), 0)`, joka laskee tuotteiden hintojen summan ja muuttaa mahdollisen `NULL`-arvon nollaksi.

Voimme toteuttaa kyselyn vähän toisella tavalla `WITH`-komennon avulla:
...
SELECT name, total, total * 0.8 AS discount FROM CustomerTotals;
```

Nyt kyselyn toteuttamista varten luodaan nimetty apukysely `CustomerTotals`, joka hakee asiakkaiden ostosten yhteishinnat. Tässä tapauksessa apukyselystä voidaan hakea tietoa kuin se olisi tietokannan taulu, jossa on seuraavat rivit:
...
Pääkysely muodostaa apukyselyn `CustomerTotals` perusteella lopullisen tulostaulun, johon tulee sekä tuotteiden yhteishinta että 20 prosentilla alennettu hinta:
...
Komennosta `WITH` voi olla hyötyä monimutkaisen kyselyn toteuttamisessa ja ymmärtämisessä, koska nimetyt apukyselyt voivat selkeyttää kyselyn rakennetta. Esimerkiksi yllä olevassa kyselyssä apukyselyn `CustomerTotals` roolina on, että se laskee asiakkaiden ostosten yhteishinnat. Tämän perusteella on helppoa muodostaa lopullinen tulostaulu, jossa näkyy myös alennettu yhteishinta. Yhteishinnan laskukaava `IFNULL(SUM(P.price), 0)` esiintyy kyselyssä vain kerran.

{: .note-title }
`WITH` vs. näkymä
<div class="note" markdown="1">
`WITH`-komennolla luotua apukyselyä voidaan käyttää vain yksittäisessä kyselyssä. Tietokantaan on mahdollista luoda myös pysyviä nimettyjä apukyselyjä eli näkymiä komennolla `CREATE VIEW`. Palaamme tähän asiaan luvussa 7.
</div>

## Esimerkkejä

Tässä osiossa on lisää näytteitä SQL:n mahdollisuuksista. Näistä ideoista on hyötyä joidenkin SQL Trainerin vaikeiden tehtävien ratkaisemisessa.
...
{: .note-title }
Miksi `MAX(COUNT(...))` ei toimi?
<div class="note" markdown="1">
Voimme mainiosti käyttää esimerkiksi seuraavaa SQL-kyselyä, joka hakee pisimmän elokuvan nimen pituuden:

```sql
SELECT MAX(LENGTH(name)) FROM Movies;
```

Tässä funktio `LENGTH` antaa nimen pituuden ja funktio `MAX` valitsee pisimmän näistä pituuksista.

Miksi ei voi käyttää vastaavasti yhdistelmää `MAX(COUNT(release_year))`?

Erona on, että `MAX` ja `COUNT` ovat molemmat koostefunktioita eikä koostefunktioita voi olla sisäkkäin. Funktio `LENGTH` ei kuitenkaan ole koostefunktio, vaan se laskee tuloksen yksittäiselle taulun sarakkeessa olevalle arvolle ja sitä voidaan käyttää koostefunktion `MAX` sisällä.
