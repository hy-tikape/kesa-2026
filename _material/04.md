...
Komennon `WITH` avulla kyselyn toteuttamista varten voidaan luoda aputaulu, joka on olemassa vain kyselyn suorituksen ajan.
...
Nyt kyselyn toteuttamista varten luodaan aputaulu `CustomerTotals`, joka ilmoittaa jokaiselle asiakkaalle ostosten yhteishinnan. Tämä taulu on olemassa vain kyselyn suorituksen ajan ja sen sisältönä on:
...
Pääkysely muodostaa aputaulun `CustomerTotals` perusteella lopullisen tulostaulun, johon tulee sekä tuotteiden yhteishinta että 20 prosentilla alennettu hinta:
...
Komennosta `WITH` voi olla hyötyä monimutkaisen kyselyn toteuttamisessa ja ymmärtämisessä, koska nimetyt aputaulut voivat selkeyttää kyselyn rakennetta. Esimerkiksi yllä olevassa kyselyssä aputaulun `CustomerTotals` roolina on, että siihen lasketaan ensin asiakkaiden yhteishinnat. Tämän perusteella on helppoa muodostaa lopullinen tulostaulu, jossa näkyy myös alennettu yhteishinta. Yhteishinnan laskukaava `IFNULL(SUM(P.price), 0)` esiintyy kyselyssä vain kerran.
