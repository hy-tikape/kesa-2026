...
Oletetaan, että taulu `Participants` sisältää osallistujien nimet ja maat:
...
Seuraava kysely etsii taulusta maan, josta on eniten osallistujia:
...
Tässä voisimme muotoilla kyselyn uudestaan niin, että komento `WITH` luo aputaulun `CountryCounts`, johon on laskettu jokaisesta maasta osallistujien määrä:
...
Aputaulu `CountryCounts` on olemassa vain kyselyn suorituksen ajan. Tässä tapauksessa taulun sisältönä on:
...
Pääkysely hakee tästä taulusta maan, josta on eniten osallistujia:
...
Komennosta `WITH` voi olla hyötyä monimutkaisen kyselyn toteuttamisessa ja ymmärtämisessä, koska komennon avulla kyselyn voi jakaa osiin, joille voi antaa kuvaavat nimet.

Erikoinen komento on _rekursiivinen_ `WITH`-komento, jossa taulu muodostetaan rekursiivisen säännön perusteella. Esimerkiksi seuraava kysely luo taulun, joka sisältää luvut 1–100:

```sql
WITH RECURSIVE numbers(x) AS (
    SELECT 1 UNION SELECT x + 1 FROM numbers WHERE x < 100
)
SELECT * FROM numbers;
```
...
Tässä kysely `SELECT 1` lisää ensimmäiselle riville luvun 1 ja kysely `SELECT x + 1 FROM numbers WHERE x < 100` lisää rekursiivisesti lisää rivejä lukuun 100 asti. Komentoa `UNION` käytetään yhdistämään nämä kyselyt samaan kokonaisuuteen.

Rekursiivisen `WITH`-komennon avulla voi toteuttaa monimutkaista laskentaa. SQLiten dokumentaatiossa on [kaksi esimerkkiä](https://sqlite.org/lang_with.html#outlandish_recursive_query_examples), joista ensimmäinen piirtää Mandelbrotin joukon ja toinen etsii sudokutehtävän ratkaisun.
