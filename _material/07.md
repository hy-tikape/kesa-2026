...
Triggeri `insert_product` aktivoituu, kun tauluun `Products` lisätään uusi rivi. Se lisää tauluun `ProductsHistory` rivin, jossa muutoksena on `insert`. Vastaavasti triggeri `update_product` aktivoituu rivin päivityksen yhteydessä ja lisää rivin, jossa muutoksena on `update`. Molemmissa triggereissä `NEW` viittaa taulun `Products` rivin sisältöön muutoksen jälkeen.
