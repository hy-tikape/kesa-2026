...
_Näkymä_ on nimetty `SELECT`-kysely, josta voidaan hakea tietoa samaan tapaan kuin tietokannassa olevasta taulusta. Idea on sama kuin `WITH`-komennon aputaulussa, mutta erona on, että näkymä on pysyvästi tallessa tietokannassa.

Näkymä luodaan komennolla `CREATE VIEW`, jossa annetaan näkymän nimi sekä kysely, jonka perusteella näkymä muodostetaan.
...
Seuraava komento luo näkymän `CustomerTotals`, joka sisältää kunkin asiakkaan ostosten yhteishinnan.

```sql
CREATE VIEW CustomerTotals AS
SELECT
  C.name,
  IFNULL(SUM(P.price), 0) AS total
FROM Customers AS C
  LEFT JOIN Purchases AS X ON C.id = X.customer_id
  LEFT JOIN Products AS P ON P.id = X.product_id
GROUP BY C.id;
```

Nyt näkymästä `CustomerTotals` voidaan hakea tietoa samaan tapaan kuin samannimisestä taulusta:
...
```sql
SELECT name FROM CustomerTotals WHERE total > 0;
```
...
Tietokannassa ei ole tallennettuna näkymässä olevia rivejä, vaan vain tieto siitä, mikä `SELECT`-kysely suoritetaan, kun näkymästä haetaan tietoa. Esimerkiksi kun yllä olevasta näkymästä `CustomerTotals` haetaan tietoa, suoritetaan taustalla `SELECT`-kysely, joka hakee tiedot tauluista `Customers`, `Purchases` ja `Products`.

### Triggerit

_Triggeri_ on komentosarja, joka suoritetaan automaattisesti, kun tietokantaan tehdään tietty muutos. Triggeri voi aktivoitua esimerkiksi, kun tiettyyn tauluun lisätään rivi tai taulussa olevaa riviä päivitetään.

Triggeri luodaan komennolla `CREATE TRIGGER`, jonka yhteydessä kuvataan, missä tilanteessa triggeri aktivoituu ja mitkä komennot se suorittaa.

Tarkastellaan esimerkkinä tilannetta, jossa haluamme pitää kirjaa siitä, miten tietokannan taulun `Products` sisältö on muuttunut. Kuten ennenkin, taulu on luotu seuraavalla komennolla:
...
Luodaan nyt toinen taulu `ProductsHistory`, johon on tarkoituksena tallentaa tiedot kaikista tauluun `Products` liittyvistä rivien muutoksista.
...
Tässä sarake `product_id` kertoo, mihin taulun `Product` riviin muutos liittyy. Sarake `action` on joko `insert` (rivin lisäys) tai `update` (rivin päivitys). Sarakkeet `name` ja `price` kertovat rivin sisällön muutoksen jälkeen.

Ideana on, että taulun `ProductsHistory` sisältö rakentuu täysin automaattisesti taulun `Products` muutosten perusteella. Tämä voidaan toteuttaa kätevästi luomalla kaksi triggeriä seuraavasti:

```sql
CREATE TRIGGER insert_product AFTER INSERT ON Products
BEGIN
  INSERT INTO ProductsHistory (product_id, action, name, price)
...
BEGIN
  INSERT INTO ProductsHistory (product_id, action, name, price)
         VALUES (NEW.id, 'update', NEW.name, NEW.price);
END;
```

Triggeri `insert_product` aktivoituu, kun tauluun `Products` lisätään uusi rivi. Se lisää tauluun `ProductsHistory` rivin, jossa muutoksena on `insert`. Vastaavasti triggeri `update_product` aktivoituu rivin pävityksen yhteydessä ja se lisää rivin, jossa muutoksena on `update`. Molemmissa triggereissä `NEW` viittaa taulun `Products` rivin sisältöön muutoksen jälkeen.

Suoritetaan sitten seuraavat komennot, jotka muuttavat taulua `Products`:

```sql
INSERT INTO Products (name, price) VALUES ('retiisi', 7);
INSERT INTO Products (name, price) VALUES ('porkkana', 5);
UPDATE Products SET price = 3 WHERE name = 'porkkana';
INSERT INTO Products (name, price) VALUES ('nauris', 4);
UPDATE Products SET price = price * 0.5;
```

Näiden komentojen seurauksena taulun `Products` sisältönä on:
...
Nyt taulusta `ProductsHistory` voidaan katsoa, millaisia muutoksia tauluun `Products` on tehty historian aikana:

```
id  product_id  action  name      price
--  ----------  ------  --------  -----
1   1           insert  retiisi   7
2   2           insert  porkkana  5
3   2           update  porkkana  3
4   3           insert  nauris    4
5   1           update  retiisi   3.5
6   2           update  porkkana  1.5
7   3           update  nauris    2
```

Tässä ensimmäiset neljä komentoa muuttivat yksittäistä riviä taulussa `Products`, jolloin triggeri lisäsi kunkin komennon kohdalla yhden rivin tauluun `ProductsHistory`. Viimeinen muutos (`SET price = price * 0.5`) sen sijaan vaikutti kaikkiin riveihin, jolloin triggeriä `update_product` kutsuttiin erikseen jokaiselle muutetulle riville ja tauluun `ProductsHistory` lisättiin kolme uutta riviä.
